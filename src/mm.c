 ...
 */
 ...
 {                                                   // no guarantee all given pages are mapped
   ...
   ret_rg->rg_end = addr + (pgit * PAGING_PAGESZ) - 1;
   ret_rg->rg_next = NULL;
 ...
 int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct **frm_lst)
 {
   ...
   for (pgit = 0; pgit < req_pgnum; pgit++)
   {
     if (MEMPHY_get_freefp(caller->mram, &fpn) == 0)
     {
       ...
       if (!temp)
       {
         ...
         while (head)
         {
           ...
           struct framephy_struct *to_free = head;
           head = head->fp_next;
           free(to_free);
         }
       ...
       if (!head)
       {
         head = temp; // Initialize head if it's the first frame
       }
       else
       {
         newfp_str->fp_next = temp; // Link the new frame to the list
       }
       newfp_str = temp;
     }
     else
     {
       ...
       return -3000; // Not enough frames
     }
   }
   *frm_lst = head;
 
   return req_pgnum;  // Return number of allocated frames
 }
...
int __swap_cp_page(struct memphy_struct *mpsrc, int srcfpn,
                   struct memphy_struct *mpdst, int dstfpn)
{
    printf("[DEBUG][SWAP ] Copying page from frame %d → frame %d\n", srcfpn, dstfpn);
    ...
    for (int cellidx = 0; cellidx < PAGING_PAGESZ; cellidx++)
    {
        ...
        MEMPHY_read(mpsrc, addrsrc, &data);
        MEMPHY_write(mpdst, addrdst, data);
        printf("[DEBUG][SWAP ] %08d → %08d | val = %d\n", addrsrc, addrdst, data);
    }
